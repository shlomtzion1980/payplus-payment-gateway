...
if (isCheckout || hasOrder) {
    ...
    // Israeli ID validation for blocks checkout
    // Function to show error with countdown animation
    function showIDErrorBlocks(message) {
    ...
    // Add validation on checkout completion for hosted fields
    ...
    wp.data.subscribe(() => {
        const isProcessing = checkoutStore.isProcessing();
        const activePaymentMethod = payment.getActivePaymentMethod();
        
        // Only validate when processing starts (not on every state change)
        if (isProcessing && !lastProcessingState && activePaymentMethod === 'payplus-payment-gateway-hostedfields') {
            const idField = document.querySelector("#id-number");
            ...
            if (idField) {
                ...
                if (id.length > 0) {
                    ...
                    if (!/^\d{9}$/.test(id)) {
                        errorMessage = 'Invalid Israeli ID number: Must be exactly 9 digits.';
                    }
                    // Validate checksum
                    else if (!validateIsraeliID(id)) {
                        errorMessage = 'Invalid Israeli ID number: Checksum validation failed.';
                    }
                    
                    if (errorMessage) {
                        // Show the custom error message
                        showIDErrorBlocks(errorMessage);
                        
                        // Add visual feedback
                        if (idField.classList) {
                            idField.classList.add('invalid');
                        }
                        const wrapper = document.querySelector("#id-number-wrapper");
                        if (wrapper && wrapper.classList) {
                            wrapper.classList.add('invalid');
                        }
                        
                        // Set validation error to stop checkout and allow retry
                        if (VALIDATION_STORE_KEY && wp.data.dispatch) {
                            wp.data.dispatch(VALIDATION_STORE_KEY).setValidationErrors({
                        ...
                        // Reset the processing state after a short delay to allow retry
                        setTimeout(() => {
                            if (wp.data.dispatch && VALIDATION_STORE_KEY) {
                                wp.data.dispatch(VALIDATION_STORE_KEY).clearValidationError('israeli-id-validation');
                            }
                        }, 100);
                    }
                }
