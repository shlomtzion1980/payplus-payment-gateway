...
if (isCheckout || hasOrder) {
    ...
    // TV screen closing effect function for blocks
    // Targets the visible payment iframe container (the one containing #pp_iframe) so it works
    // when multiple .pp_iframe exist and on remote where DOM order may differ. Uses stylesheet
    // (style.css) for keyframes so CSP on remote does not block inline styles.
