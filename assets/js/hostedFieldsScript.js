...
hf.Upon("pp_responseFromServer", (e) => {
    ...
    if (e.detail.errors) {
        ...
        // Show red X on card number field for card-related errors
        var ccErrors = ["credit-card-number-is-incorrect", "card-holder-vat-id-not-valid", "missing-cc"];
        var errorField = e.detail.errors[0].field || "";
        var errorMsg = e.detail.errors[0].message || "";
        if (errorField === "cc" || ccErrors.indexOf(errorMsg) !== -1) {
            jQuery("#cCard").removeClass("validated").addClass("invalid");
        }
    }
...
hf.Upon("pp_ccTypeChange", (e) => {
    ...
    if (ccType && ccType !== "" && ccType !== "unknown") {
        $cCard.removeClass("invalid").addClass("validated");
    }
});
