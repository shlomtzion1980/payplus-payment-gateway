jQuery(document).ready(function ($){
   // setFieldReadOnly();
    ...
    $(".select-languages-payplus").change(function (event){
        ...
        if(language) {
            ...
            html = '<tr valign="top">' +
                ...
                '<label for="settings_payplus_page_error_option[' + language + ']">' + languageOther[1] + '</label>' +
                '</th>' +
                '<td class="forminp forminp-textarea">' +
                '<textarea name="settings_payplus_page_error_option[' + language + ']" id="settings_payplus_page_error_option[' + language + ']" ' +
    ...
    $("#payplus-token-payment").click(function (event){
        ...
        $(this)
            .closest('.delayed-payment').find(".payplus_loader").fadeIn();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(this)
                    .closest('.delayed-payment').find(".payplus_loader").fadeOut();
                if(!response.status){
...
function  PayplusdisplayMenuInvoice(){
    ...
    if(section=='payplus-invoice' || section=='payplus-express-checkout' || section=='payplus-error-setting'){
        jQuery('.wrap.woocommerce').find("h2").before(payplus_script_admin.menu_option);
    }
}
