...
async function createNewShippingMethods() {
    ...
    jQuery("#shipping_method li").each(function () {
        ...
        const bdi = jQuery(this).find("bdi");
        ...
        session.completeShippingContactSelection({
            ...
            newTotal: {
                ...
                amount:
                    ...
                    (globalPayingVat
                        ? parseFloat(currentShippingTax) +
                          parseFloat(globalTaxForProducts)
                        : 0),
            },
...
function onShippingMethodSelected(event, session) {
    ...
    if (checkShipping.length === 0 && applePayConfig.shippingMethods.length) {
        ...
        currentShippingIdentifier =
            applePayConfig.shippingMethods[0].identifier;
    } else {
        ...
        await jQuery.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    session.completePayment(ApplePaySession.STATUS_FAILURE);
                    displayMsgError(
                        response.payment_response.results.description
                    );
                }
            },
            error: function (jqXHR, textStatus, errorThrow) {
...
function checkArrayType(product) {
    ...
    if (Array.isArray(productData)) {
        ...
        if (
            typeof productData[0] === "object" &&
            !Array.isArray(productData[0])
        ) {
            return 3;
...
window.addEventListener("message", async function (event) {
    ...
    if (senderOrigin === domain) {
        ...
        if (paymentData.oneClickCheckoutGooglePay === "ProcessPayment") {
            ...
            paymentData.oneClickCheckoutGooglePay === "getCurrentPrice"
        ) {
            displayMsgError();
            let priceUpdatedNumber;
            let products = [];
            let currency_code = document.getElementById(
                "payplus_currency_code"
            );
            ...
            let shipping;
            let shippingWoo =
                document.getElementById("payplus_shipping_woo").value ===
                "true";
            if (shippingWoo) {
            ...
            displayMsgError(
                "Server failure, please contact the site administrator"
            );
        }
    }
});
