window.addEventListener('load', function () {
    ...
    let titleExpressCheckout =  document.querySelector('.title-express-checkout span');
...
let productID = googleButton ? googleButton.getAttribute("data-product-id") :
    (appleButton1 ? appleButton1.getAttribute("data-product-id") : false)
//APPLE PAY
...
const payment_url_google_pay_iframe = payplus_script.payment_url_google_pay_iframe;
let textError = 'error : '

const applePayScript = document.createElement('script')
applePayScript.setAttribute('src', 'https://applepay.cdn-apple.com/jsapi/v1/apple-pay-sdk.js')
...
function updateApplePayConfig(formattedtShippingArray, formattedtShippingArrayPayPlus) {
    applePayConfig.shippingMethods = formattedtShippingArray;
    currentShippingPrice = formattedtShippingArray.length ? formattedtShippingArray[0]?.amount : 0;
    currentShippingTax = formattedtShippingArrayPayPlus.length ? formattedtShippingArrayPayPlus[0]?.tax_amount : 0;
    currentShippingIdentifier = formattedtShippingArray.length ? formattedtShippingArray[0]?.identifier : '';
}
...
function updateLineItems() {
    ...
    if (globalDiscount) {
        newLineItems.push({"label": "Discount", "type": "final", "amount": -globalDiscount});
    }
    newLineItems.push({"label": "Shipping", "type": "final", "amount": currentShippingPrice});
    newLineItems.push({
        "label": "Estimated Tax",
        "type": "final",
...
function calculateTotal() {
    const newTotal = calculateNewTotal((globalPriceProductsWithoutTax - globalDiscount), currentShippingPrice);
    let total = ((parseFloat(newTotal)) + (globalPayingVat ? (globalTaxForProducts + currentShippingTax) : 0));
...
async function onShippingContactSelected(event, session) {
    try {
        ...
        if (countryCode) {
            ...
            formattedtShippingArrayPayPlus = arrayShipping.newShippingOptionsForPayPlus;
            currentShippingArrayPayPlus = arrayShipping.newShippingOptionsForPayPlus;
        }

        updateApplePayConfig(formattedtShippingArray, formattedtShippingArrayPayPlus);
        session.completeShippingContactSelection({
            newShippingMethods: formattedtShippingArray,
            newTotal: {
                label: 'Total',
                type: 'final',
                amount: globalPriceProductsWithoutTax + parseFloat(currentShippingPrice) + (globalPayingVat ? parseFloat(currentShippingTax) + parseFloat(globalTaxForProducts) : 0)
            },
...
function onShippingMethodSelected(event, session) {
    ...
    const checkShipping = applePayConfig.shippingMethods.filter(item => item.identifier === selectedShippingMethod.identifier);
    let shippingAmount;
    ...
    const getShippingWithTaxObj = currentShippingArrayPayPlus.filter(item => item.identifier === currentShippingIdentifier);
...
async function onPaymentAuthorized(event, session) {
    try {
        ...
        if (!event.payment.shippingContact ||
            !event.payment.shippingContact.familyName || !event.payment.shippingContact.givenName ||
...
function formattedShipping(countryCode, withTax = false) {
    let allShipping
    let shippingWoo = document.getElementById('payplus_shipping_woo').value === 'true'
    if (shippingWoo) {
        allShipping = document.getElementById('payplus_shipping').value
        allShipping = JSON.parse(allShipping)
    } else {
        const cost_shipping_with_tax = document.getElementById('payplus_pricewt_shipping').value
        const cost_shipping_without_tax = document.getElementById('payplus_pricewithouttax_shipping').value
        allShipping = {
            all: [{
                id: 0,
    ...
    if (allShipping) {
        ...
        newShippingOptionsForApple = shippingOptions.map((item) => ({
            ...
            label: item.title,
            detail: 'shipping',
            amount: withTax ? (item.cost_with_tax || '0').toString() : (item.cost_without_tax || '0').toString()
        }))
        ...
        newShippingOptionsForPayPlus = shippingOptions.map((item) => ({
            ...
            detail: 'shipping',
            amount: withTax ? (item.cost_with_tax || '0').toString() : (item.cost_without_tax || '0').toString(),
            tax_amount: parseFloat(item.cost_with_tax || 0) - parseFloat(item.cost_without_tax || 0)
        }))
    }
