...
jQuery(function ($) {
  ...
  jQuery(function ($) {
    $(document.body).on("updated_checkout", function () {
      const customButton = $("#custom-place-order-"); // Match your dynamic ID.

      if (customButton.length) {
        customButton.on("click").on("click", function () {
          $('form[name="checkout"]').trigger("submit"); // Submits the checkout form
        });
      }
      jQuery("#submit-payment").on("click", function () {
        jQuery('form[name="checkout"]').trigger("submit"); // Submits the checkout form
      });
    });
  });
                ...
                success: function (response) {
                  ...
                  if (
                    ...
                    if (totalElement) {
                      ...
                      let totalAmount = totalText.replace(/[^0-9.]/g, "");
                      //   console.log(totalAmount);
                      if (
                        Number(hostedPayload.amount) === Number(totalAmount)
                      ) {
                        overlay();
      ...
      if (element) {
        ...
        if (imageToChange) {
          ...
          function loopReplaceImageSource() {
            ...
            if (
              Object.keys(payplus_script_checkout.multiPassIcons).length > 1
            ) {
