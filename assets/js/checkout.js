...
jQuery(function ($) {
    ...
    setTimeout(function () {
        ...
        if (payplus_script_checkout.isAutoPPCC && hasSavedCCs.length === 0) {
            ...
            $("#wc-payplus-payment-gateway-new-payment-method").trigger(
                "click"
            );
    ...
    if (payplus_script_checkout.isHostedFields) {
        $("li.wc_payment_method.payment_method_payplus-payment-gateway").hide();
        $("#payment_method_payplus-payment-gateway-hostedfields").prop(
            "checked",
            true
        );
        setTimeout(function () {
