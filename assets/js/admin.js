...
jQuery(function ($) {
    ...
    if (listHidden.length) {
        ...
        if (listHidden.length) {
            for (let i = 0; i < listHidden.length; i++) {
                ...
                $.each(
                    $(".invoice-manual-list").children("option"),
                    function () {
    ...
    // remove tooltips for subgateways //
    // List of subgateway IDs for which you want to hide the tooltip
    var subgateways = [
    ...
    // Iterate through each subgateway and hide the tooltip
    ...
    if (typeof payplus_script_payment !== "undefined") {
        ...
        const taxMessage =
            currentLanguage === "he"
                ? "שימו לב: שיעורי המס והחישובים מופעלים באתר זה - חלק ההגדרות הרשומות מעלה לא יהיו ברות השפעה."
    ...
    shippingwoo.change(function () {
        ...
        if (!checked) {
            ...
            if (
                enableGooglePay.prop("checked") ||
                enableApplePay.prop("checked")
            ) {
                globalShipping.attr("required", "required");
                globalShippingTax.attr("required", "required");
                globalShippingTaxRate.attr("required", "required");
    ...
    if ($("div#payplus-options").length) {
        ...
        if (
            $("#payplus_invoice_option\\[payplus_invoice_enable\\]").prop(
                "checked"
            )
        ) {
            let ids = [
    ...
    $(document).on("input", ".price", function (e) {
        ...
        if (m_price) {
            ...
            if (
                document.querySelector(".payplus_payment").style.display ==
                "flex"
            ) {
                payplus_set_payments(classs[0]);
            } else {
    ...
    $(document).on("click", ".payplus-payment-button", function (e) {
        ...
        let collections = Array.prototype.filter.call(
            collection,
            (collection) =>
                collection.nodeName === "INPUT" ||
                collection.nodeName === "SELECT"
        );
    ...
    $(document).on("change", ".select-type-invoice", function (event) {
        ...
        if (value == "inv_receipt" || value == "inv_tax_receipt") {
            document.getElementById("all-payment-invoice").style.display =
                "block";
            document.getElementById("payplus-table-payment").style.display =
                "table";
            document.getElementById("payplus_sum_payment").style.display =
                "block";
        } else {
            document.getElementById("all-payment-invoice").style.display =
                "none";
    ...
    $(document).on("click", "#payplus-create-invoice", function (event) {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                alert(
                    "Error: Either you do not have permission or unknown error."
                );
                //  location.reload();
            },
        });
...
function payplus_empty_field() {
    ...
    for (let i = 0; i < collections.length; i++) {
        ...
        if (
            classs[2] != "method_payment" &&
            classs[2] != "number_of_payments"
        ) {
            collections[i].value = "";
        }
    }
...
function payplus_set_max_total(flag = true) {
    ...
    if (orderID) {
        if (price && allSum) {
            let payplus_sum_payment = document.getElementById(
                "payplus_sum_payment"
            );
    ...
    if (orderID) {
        ...
        if (payments[index]) {
            ...
            const method_payment =
                objectPayment.method_payment + "-payment-payplus";
            ...
            for (let i = 0; i < collections.length; i++) {
                const classs = collections[i].classList;
                document.querySelector(
                    "." + method_payment + "." + classs[2]
                ).value = objectPayment[classs[2]];
                if (classs[2] == "price") {
                    let sum =
                        allSum.value -
                        payplus_get_sum_payments(objectPayment.row_id);
                    sum = sum.toFixed(2);
                    document
            ...
            jQuery(".type-payment").removeClass("hover");
            jQuery(
                "." + objectPayment.method_payment + ".type-payment"
            ).addClass("hover");
