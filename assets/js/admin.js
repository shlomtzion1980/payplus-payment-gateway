...
jQuery(function ($) {
    ...
    const transactionType =$("#woocommerce_payplus-payment-gateway_transaction_type");
    const checkAmountAuthorization=$("#woocommerce_payplus-payment-gateway_check_amount_authorization");
    const blockIpTransactions=$("#woocommerce_payplus-payment-gateway_block_ip_transactions");
    const blockIpTransactionsHour=$("#woocommerce_payplus-payment-gateway_block_ip_transactions_hour");
    const changeVatInEliat=$("#woocommerce_payplus-payment-gateway_change_vat_in_eilat");
    const keywordsEilat=$("#woocommerce_payplus-payment-gateway_keywords_eilat");
    ...
    if(listHidden.length){
        ...
        if(listHidden.length){
            ...
            $(".invoice-manual-list").children("option:first").prop("selected",false);
        }
    ...
    if (shippingwoo && !shippingwoo.prop("checked") && globalShipping && !globalShipping.val()) {
    ...
    enableGooglePay.change(function (event) {
        ...
        if (checked) {
            ...
            $.ajax({
                ...
                data: { action: "payplus-express-checkout-initialized", method: "google-pay" },
                success: function (response) {
                    elementFieldset.find(".loading-express").fadeOut();
                    if (!response.status) {
                        elementFieldset.find(".error-express-checkout").html("<b>payplus error : </b>" + response.response_initialized.results.description);
    ...
    enableApplePay.change(function () {
        ...
        if (checked) {
            ...
            $.ajax({
                ...
                dataType: "json",
                url: payplus_script_admin.ajax_url,
                data: { action: "payplus-express-checkout-initialized", method: "apple-pay" },
                success: function (response) {
                    elementFieldset.find(".loading-express").fadeOut();
                    if (!response.status) {
                        let description =(response.response_initialized.results.description.description)?
                            response.response_initialized.results.description.description :response.response_initialized.results.description;
                        elementFieldset.find(".error-express-checkout").html("<b> payplus error : </b>" + description);
                        slef.prop("checked", false);
                    } else {
                        tokenApplePay.parents("tr").fadeIn();
                        tokenApplePay.val(response.response_initialized.apple_pay_identifier);
                    }
                },
            });
        } else {
    ...
    $(document).on("click",".type-payment",function (event){
        ...
        document.querySelectorAll('.select-type-payment').forEach( function (item, idx) {
            if(item.style.display=="block"){
                rowId =document.querySelector('.select-type-payment.'+item.classList[1] +' .row_id').value;
            }
            item.style.display='none';
        });
        $(".type-payment").removeClass('hover')
        $(this).addClass('hover');
        document.querySelector(".select-type-payment"+"."+type).style.display='block';
        document.querySelector(".select-type-payment"+"."+type+ " .row_id").value=rowId;
    ...
    $(document).on("input",".price",function (e){
        ...
        if(m_price){
            if(parseFloat(m_price) < 0 ||parseFloat( m_price_max) < parseFloat(m_price)){
    ...
    $(document).on("click",".payplus-paymnet-button",function (e){
        ...
        let payments =  JSON.parse( localStorage.getItem('payplus_payment_'+orderID));
        let collection = document.getElementsByClassName(id);
        let collections = Array.prototype.filter.call(
            collection,
            (collection) => collection.nodeName === "INPUT" || collection.nodeName === "SELECT",
        );
        ...
        if(data['price'] >0 && data['price']!=""){
            ...
            localStorage.setItem('payplus_payment_' + orderID, JSON.stringify(payments));
            payplus_empty_field();
            payplus_print_paymnets_all();
            payplus_set_max_total();
            document.querySelectorAll('.select-type-payment').forEach( function (item, idx) {
    ...
    $(document).on("click","#payplus-create-invoice",function (event){
        ...
        if(allSum && (typeDocument =='inv_receipt'  || typeDocument=='inv_tax_receipt')){
...
function payplus_empty_field(){
    ...
    const collections =document.querySelectorAll('#all-paymnet-invoice input,#all-paymnet-invoice select');
...
function payplus_set_max_total(flag=true){
    ...
    if(orderID) {
        ...
        if(price && allSum) {
            ...
            if(select_type_invoice.value =='inv_receipt'  || select_type_invoice.value=='inv_tax_receipt'){
                document.getElementById('all-paymnet-invoice').style.display ='block';
...
function payplus_get_sum_payments(rowId =-1){
    ...
    let  sum =0;
    if(orderID){
        const payments = JSON.parse(localStorage.getItem('payplus_payment_' + orderID.value));
        if(payments){
            if(rowId!=-1) {
...
function payplus_print_paymnets(data,index){
    let details='';
    let detailsAll =['bank_number','account_number','branch_number' ,'check_number',
    ...
    Object.keys(data).forEach(function(key, index) {
        if(detailsAll.includes(key) && data[key] !=undefined && data[key]!="" ){
            ...
            if((key=='first_payment' || key=='subsequent_payments' || key =='number_of_payments')
                && (data['transaction_type']!="payments" && data['transaction_type']!='credit')){
    ...
    const  pricePayment =data && data.price ? Number(parseFloat(data.price).toFixed(2)):0;
    ...
    let html =`<tr class="row-paymnet-${data.row_id}">
        <td>
        <a class="link-action" data-id=${data.row_id} onclick="payplus_delete_element(${data.row_id})">${payplus_script_payment.btn_delete}</a>
        <a class="link-action" data-id=${data.row_id} onclick="payplus_edit_element(${data.row_id})">${payplus_script_payment.btn_edit}</a>
        </td>
         <td><span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">
                    ${payplus_script_payment.currency_symbol}</span>${pricePayment}</bdi></span>
                 </td>
                    <td>${details}</td>
...
function payplus_print_paymnets_all() {
    ...
    if (orderID && table_payment) {
        ...
        if (payments) {
            ...
            table_payment.html('');
            if (payments.length) {
                let html = `<strong>${payplus_script_payment.payplus_sum} : ${payplus_script_payment.currency_symbol}${payplus_get_sum_payments()}  </strong>`
...
function payplus_delete_element(index ,flag=false){
    ...
    if(flagDelete){
        ...
        const orderID = document.getElementById("post_ID");
        if(orderID){
            let payments =  JSON.parse( localStorage.getItem('payplus_payment_'+orderID.value));
            payments =payments.filter(x=>x.row_id !=index);
            if(!payments.length ){
                payments=[];
            }
            localStorage.setItem('payplus_payment_'+orderID.value , JSON.stringify(payments));
            payplus_print_paymnets_all();
...
function payplus_edit_element(index){
    ...
    if(orderID){
        ...
        if(payments[index]){
            ...
            const elemnetDisplay =document.querySelector(".select-type-payment." +objectPaymnet.method_payment)
            ...
            for (let i=0; i<collections.length; i++) {
                const  classs =collections[i].classList;
                document.querySelector('.'+method_payment+"."+classs[2]).value =objectPaymnet[classs[2]];
                if(classs[2]=="price"){
                    let  sum= allSum.value -payplus_get_sum_payments (objectPaymnet.row_id);
                    sum =sum.toFixed(2);
                    document.querySelector('.'+method_payment+"."+classs[2]).setAttribute('max',sum);
                    for (let  j=0; j<price.length; j++) {
                        price[j].value =sum
                    }
                    document.querySelector('.'+method_payment+"."+classs[2]).value =objectPaymnet['price'];
                    for (let  j=0; j<fullAmount.length; j++) {
                        fullAmount[j].setAttribute('data-sum', sum);
                    }
                }
            }
            if( objectPaymnet.transaction_type=="payments" ||  objectPaymnet.transaction_type=='credit'){
                document.querySelector('.payplus_payment').style.display='flex';
            }else{
                document.querySelector('.payplus_payment').style.display='none';
            }
            jQuery(".type-payment").removeClass('hover')
            jQuery("." +objectPaymnet.method_payment +'.type-payment').addClass('hover');
...
function payplus_get_storage(orderID) {
    const payments = JSON.parse(localStorage.getItem('payplus_payment_' + orderID));
    return payments;
