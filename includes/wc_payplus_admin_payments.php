...
class WC_PayPlus_Admin_Payments extends WC_PayPlus_Gateway
{
    ...
    public function custom_action_callback()
    {
        ...
        $payplusResponse = get_post_meta($order_id, 'payplus_response', true);

        if (!$payplusResponse) {
            payplus_update_post_meta_object($order, array('payplus_response' => json_encode($responseBody['data'])));
        }

        if (!get_post_meta($order_id, 'payplus_type', true)) {
            payplus_update_post_meta_object($order, array('payplus_type' => $responseBody['data']['type']));
        }
    ...
    public function payplus_get_error_setting($settings, $current_section)
    {
        ...
        switch ($current_section) {
            ...
            case 'payplus-invoice':
                ...
                $selectTypeDoc = array('' => __('Type Documents', 'payplus-payment-gateway'),
                ...
                $settings[] = array(
                    ...
                    'desc' => '<span class="arrow-payplus">Add a unique string here if you have more than one website connected to the service <br> This will create a unique id for invoices to each site (website code must be different for each site!)</span>',
                );
                $settings[] = array(
            ...
            case 'payplus-express-checkout':
                ...
                $settings[] =
                array('id' => 'woocommerce_payplus-payment-gateway_settings[global_shipping]',
                    'name' => __('Global shipping amount', 'payplus-payment-gateway') . ' ( ' . get_woocommerce_currency_symbol() . ' ) ',
                    'type' => 'text',
                    'class' => 'global_shipping',
                    'default' => '0');

                $settings[] = array('id' => 'woocommerce_payplus-payment-gateway_settings[global_shipping_tax]',
                ...
                if (count($rates)) {
                    $settings[] = array('id' => 'woocommerce_payplus-payment-gateway_settings[global_shipping_tax_rate]',
    ...
    public function ajax_payplus_refund_club_amount()
    {
        if (!empty($_POST)
            ...
            $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'WP Refund  club card(' . $orderID . ')');
            ...
            if ($WC_PayPlus_Gateway->invocie_api->payplus_get_invoice_enable()) {
                $payload['initial_invoice'] = false;
            }

            $payload = json_encode($payload);
            $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($payload, true), 'payload');
            $response = $WC_PayPlus_Gateway->post_payplus_ws($WC_PayPlus_Gateway->refund_url, $payload);
            if (is_wp_error($response)) {
                ...
                if ($res->results->status == "success" && $res->data->transaction->status_code == "000") {
                    payplus_update_post_meta_object($order, array('payplus_total_refunded_amount' => round($refunded_amount + $amount, 2)));
                    ...
                    /* invoice api*/
                    if ($WC_PayPlus_Gateway->invocie_api->payplus_get_invoice_enable() &&
                        !$WC_PayPlus_Gateway->invocie_api->payplus_get_create_invocie_manual()) {
                        $payments = $this->payplus_get_order_payment(false, $id);
                        if ($payments[$indexRow]->price > round($amount, $WC_PayPlus_Gateway->rounding_decimals)) {
                            $payments[$indexRow]->price = $amount * 100;
                        }
                        $rand = rand(0, intval($orderID));
                        $WC_PayPlus_Gateway->invocie_api->payplus_create_dcoment_dashboard($orderID,
                            $WC_PayPlus_Gateway->invocie_api->payplus_get_invoice_type_document_refund(), $payments, round($amount, $WC_PayPlus_Gateway->rounding_decimals), 'payplus_order_refund_' . $rand . "_" . $orderID);
                    }
                    /* end invoice api**/
    ...
    public function ajax_payplus_create_invoice()
    {
        ...
        if (!empty($_POST) && !empty($_POST['order_id'])) {
            ...
            $payplusInvoice->payplus_invoice_create_order($order_id, $type_document);
            echo json_encode(array("urlredirect" => $urlEdit, "status" => true));
            wp_die();
    ...
    public function ajax_payplus_create_invoice_refund()
    {
        if (!empty($_POST) && !empty($_POST['order_id'])) {
            ...
            $resultApps = $payplusInvoice->payplus_get_payments($order_id);
            ...
            if (count($resultApps)) {
                ...
                $payplusInvoice->payplus_create_dcoment_dashboard($order_id,
                    $type_document, $resultApps, round($amount, $WC_PayPlus_Gateway->rounding_decimals), 'payplus_order_refund' . $order_id . "_" . rand(1, 1000));
                $wpdb->update($table_name, array('invoice_refund' => 0), array('order_id' => $order_id));
                if ($amount == $order->get_total()) {
                    payplus_update_post_meta_object($order, array('payplus_send_refund' => true));
                }
            }
            echo json_encode(array("urlredirect" => $urlEdit, "status" => true));
            wp_die();
            ...
            if ($order) {
                ...
                if ($status == "shop_subscription") {
                    ...
                    if ($order->get_user() != false) {
                        ...
                        if (empty($token)) {
                            $transaction_uid = get_post_meta($parent_id, 'payplus_transaction_uid', true);
                            if ($transaction_uid) {
                            ...
                            $payload = json_encode($payload);
                            $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'New IPN Fired (' . $order_id . ')');
                            $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($payload, true), 'payload');
                            $data['order_id'] = $order_id;
                            $returnIpn = $WC_PayPlus_Gateway->requestPayPlusIpn($payload, $data, 1, $handle, true);
                            ...
                            if ($returnIpn->results->status == 'success') {
                                ...
                                if ($StatusCode == "000") {
                                    ...
                                    payplus_update_post_meta_object($order, array('payplus_token_uid' => $token));
                            ...
                            payplus_update_post_meta_object($order, array('order_validated' => "1"));
                    ...
                    $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'New IPN Fired (' . $order_id . ')');
                    $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($payload, true), 'payload');
                    $WC_PayPlus_Gateway->requestPayPlusIpn($payload, array('order_id' => $order_id), 1);
                    payplus_update_post_meta_object($order, array('order_validated' => '1'));
    ...
    public function ajax_payplus_generate_link_payment()
    {
        ...
        if (!empty($_POST)) {
            ...
            $order_id = $_POST['order_id'];
            $order = wc_get_order($order_id);
            $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'New Payment Process Fired (' . $order_id . ')');
            $payload = $WC_PayPlus_Gateway->generatePayloadLink($order_id, true);
            $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($payload, true), 'payload');
            $response = $WC_PayPlus_Gateway->post_payplus_ws($WC_PayPlus_Gateway->payment_url, $payload);
            ...
            if (is_wp_error($response)) {
                ...
                $res = json_decode(wp_remote_retrieve_body($response));
                if (isset($res->data->payment_page_link) && $WC_PayPlus_Gateway->validateUrl($res->data->payment_page_link)) {
                    $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($res, true), 'completed');
                    $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'WS Redirecting to Page: ' . $res->data->payment_page_link . "\n" . $this->payplus_get_space());
                    payplus_update_post_meta_object($order, array('payplus_page_request_uid' => $res->data->page_request_uid));
                    payplus_update_post_meta_object($order, array('payplus_payment_page_link' => $res->data->payment_page_link));
    ...
    public function payplus_get_section_invocie_not_automatic($orderId)
    {
        ...
        $selectInvoice = array(
            ...
            'inv_don_receipt' => __('Donation Reciept', 'payplus-payment-gateway'));
        $payplusInvoice = new PayplusInvoice();
        $invoiceManualList = $payplusInvoice->payplus_get_invoice_manual_list();
        $currentStatus = $payplusInvoice->payplus_get_invoice_type_document();
        $chackStatus = array('inv_receipt', 'inv_tax_receipt');
        $chackAllPaymnet = in_array($currentStatus, $chackStatus) ? "block" : 'none';
        $payments = $WC_PayPlus_Gateway->invocie_api->payplus_get_payments($orderId);
        $checkInvocieSend = get_post_meta($orderId, 'payplus_check_invoice_send', true);
        ...
        <div class="flex-row">
            <div class="flex-item">
                <select id="select-type-invoice-<?php echo $orderId ?>" class="select-type-invoice" name="select-type-invoice-<?php echo $orderId ?>" >
                    <option value=""><?php echo __('Select a document type to create an invoice', 'payplus-payment-gateway') ?></option>
                    <?php
                ...
                <div class="select-type-payment credit-card">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            ...
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="credit-card-payment-payplus input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div class="flex-item">
                            <label> <?php echo __("Credit card number", "payplus-payment-gateway") ?></label>
                            <input   class="credit-card-payment-payplus input-change four_digits"  type="number" onkeypress="if (value.length == 4) return false;"
                                     placeholder="<?php echo __("Four Digits", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div class="flex-item">
                        ...
                        <div  class="flex-item full-amount" >
                            ...
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="credit-card-payment-payplus input-change price"   type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>"
                                       value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount credit-card-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                    </div>
            ...
            ?>
                ...
                <div class="select-type-payment cash">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="cash-payment-payplus input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>"
                                       class="cash-payment-payplus input-change price"   type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>"
                                       value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>"
                                         class="payplus-full-amount cash-payment-payplus""> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                ...
                <div class="select-type-payment payment-check">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="payment-check-payment-payplus  input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="payment-check-payment-payplus input-change price"
                                       type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>"  value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount payment-check-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                        <div class="flex-item">
                ...
                <div class="select-type-payment bank-transfer">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="bank-transfer-payment-payplus  input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="bank-transfer-payment-payplus input-change price"
                                       type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>" value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?> " class="payplus-full-amount bank-transfer-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                        <div class="flex-item">
                            <label> <?php echo __("Bank number", "payplus-payment-gateway") ?></label>
                ...
                <div class="select-type-payment payment-app">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="payment-app-payment-payplus input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="payment-app-payment-payplus input-change price"   type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>"
                                       value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount payment-app-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                        <div class="flex-item">
...
foreach ($installed_payment_methods as $installed_payment_method): ?>
                                    <option value="<?php echo $installed_payment_method ?>"
                                    ><?php echo $installed_payment_method ?>
                                    </option>
                                <?php
endforeach;
            ?>
                ...
                <div class="select-type-payment paypal">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="paypal-payment-payplus  input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="paypal-payment-payplus input-change price"
                                       type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>"  value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount paypal-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                        <div class="flex-item">
                ...
                <div class="select-type-payment withholding-tax">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="withholding-tax-payment-payplus input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"   min="1"  max="<?php echo $order->get_total() ?>" class="withholding-tax-payment-payplus input-change price"
                                       type="number" placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>" value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount withholding-tax-payment-payplus" > <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                ...
                <div class="select-type-payment other">
                    ...
                    <div class="flex-row">
                        <div class="flex-item">
                            <label> <?php echo __("Date", "payplus-payment-gateway") ?></label>
                            <input value="<?php echo date("Y-m-d") ?>"  required  class="other-payment-payplus  input-change create_at"  type="date"
                                   placeholder="<?php echo __("Date", "payplus-payment-gateway") ?>"   >
                        </div>
                        <div  class="flex-item full-amount" >
                            <label><?php echo __("Sum", "payplus-payment-gateway") ?></label>
                            <div class="flex-row">
                                <input data-sum="<?php echo $order->get_total() ?>" step="0.01"
                                       min="1"  max="<?php echo $order->get_total() ?>" class="other-payment-payplus input-change price"   type="number"
                                       placeholder="<?php echo __("Sum", "payplus-payment-gateway") ?>" value="<?php echo floatval($order->get_total()) ?>">
                                <button  data-sum="<?php echo $order->get_total() ?>" class="payplus-full-amount other-payment-payplus"> <?php echo __("Full Amount", "payplus-payment-gateway") ?> </button>
                            </div>
                        </div>
                        <div class="flex-item">
                            <label> <?php echo __("Transaction id", "payplus-payment-gateway") ?></label>
        ...
        ?>
            <div class="flex-row">
                <div class="flex-item">
                    <button  id="payplus-create-invoice" data-id="<?php echo $orderId ?>"
                             class="button  button-primary"><span class="refund_text"><?php echo __("Create Invoice", "payplus-payment-gateway") ?></span></button>
                </div>
            </div>
        <?php
endif;
    ...
    public function payplus_get_table_paypment($orderId, $currentStatus, $payments)
    {
        ...
        $detailsAll = ['bank_number', 'account_number', 'branch_number', 'check_number',
        ...
        foreach ($payments as $key => $payment) {
            ...
            $orderAmount = get_post_meta($orderId, 'payplus_charged_j5_amount', true) ? get_post_meta($orderId, 'payplus_charged_j5_amount', true) : $payment->price / 100;
                ...
                <tr>
                    ...
                    <td>
                        <span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">
                                    <?php echo get_woocommerce_currency_symbol() ?></span><?php echo $orderAmount ?></bdi></span>
                    </td>
...
foreach ($payment as $key => $value) {
    ...
    public function add_custom_button_to_order($order)
    {
        if ($order->get_status() == 'pending') {
            $payplusResponse = get_post_meta($order->get_id(), 'payplus_response', true);
            $pageRequestUid = get_post_meta($order->get_id(), 'payplus_page_request_uid', true);
    ...
    public function payplus_woocommerce_admin_order_totals_after_total($orderId)
    {
        ...
        $payplusInvoice = new PayplusInvoice();
        $invocie_manual = $payplusInvoice->payplus_get_create_invocie_manual();
        $checkInvocieSend = get_post_meta($orderId, 'payplus_check_invoice_send', true);
        $resultApps = $payplusInvoice->payplus_get_payments($orderId, 'otherClub');
        $checkInvocieRefundSend = get_post_meta($orderId, 'payplus_send_refund', true);
        $sum = 0;
        $sumTransactionRefund = array_reduce($resultApps, function ($sum, $item) {
        ...
        $payplus_related_transactions = get_post_meta($orderId, 'payplus_related_transactions', true);
        $payplus_response = json_decode(get_post_meta($orderId, 'payplus_response', true));
        $payplus_response = (array) $payplus_response;
        $selectInvoiceRefund = array('' => __('Type Documents Refund', 'payplus-payment-gateway'),
            'inv_refund' => __('Credit Invoice', 'payplus-payment-gateway'),
            'inv_refund_receipt' => __('Credit Receipt', 'payplus-payment-gateway'),
            'inv_refund_receipt_invoice' => __('Credit Invoice + Credit Receipt', 'payplus-payment-gateway'));
        ob_start();
...
$result = $this->payplus_get_order_payment($orderId);
            if (!count($result)) {
                if (count($payplus_response)) {
                    $WC_PayPlus_Gateway->payplus_add_order($orderId, $payplus_response);
                } else {
                    $transaction_uid = get_post_meta($orderId, 'payplus_transaction_uid', true);
                    ...
                    $res = $WC_PayPlus_Gateway->requestPayPlusIpn($payload, $data, 1, 'payplus_process_payment', true);
                    payplus_update_post_meta_object($order, array('payplus_response' => json_encode($res->data, true)));
                    $WC_PayPlus_Gateway->payplus_add_order($orderId, (array) $res->data);
        ...
        if ($order->get_status() != 'auto-draft' && $order->get_status() != 'on-hold' && $total && $payplusInvoice->payplus_get_invoice_enable()
            && $invocie_manual) {
        ...
        if ($total && $payplusInvoice->payplus_get_invoice_enable()
            ...
            ?>
                ...
                <button id="payplus-create-invoice-refund" data-id="<?php echo $orderId ?>"
                        class="button  button-primary"><span class="refund_text"><?php echo __("Create Invoice refund", "payplus-payment-gateway") ?></span></button>
        ...
        if (($order->get_type() === "shop_subscription" && $order->get_status() === "on-hold")
            ...
            <div class="payment-order-ajax">
                <button id="payment-payplus-transaction" data-id="<?php echo $orderId ?>"
                        class="button  button-primary"><?php echo __("Transaction review", "payplus-payment-gateway") ?></button>
        ...
        if (!$flagPayment && !$order_validated_error && empty($checkInvocieSend)) {
            ?>
            <div class="payment-order-ajax">
                <button id="payment-payplus-dashboard" data-id="<?php echo $orderId ?>"
                        class="button  button-primary"><?php echo __("Payment", "payplus-payment-gateway") ?></button>
                <div class='payplus_loader'></div>
            </div>
        ...
        if ($WC_PayPlus_Gateway->log_status && count($logs)): ?>
            <div class="flex">
...
endforeach;
    ...
    public function ajax_payplus_token_payment()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($amount > $order->get_total()) {
                $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'Cannot Charge then original order');
            ...
            if ($OrderType != "Approval" and $OrderType != "Check") {
                $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'ansaction Not J5 Or Changed to J4 After Charge');
            ...
            if ($amount == $order->get_total() and $charged_amount == 0) {
                $chargeByItems = true;
                $objectProducts = $WC_PayPlus_Gateway->payplus_get_products_by_order_id($order_id, true);
            }
            $totalCartAmount = $objectProducts->amount;
            $payplusRefunded = get_post_meta($order_id, 'payplus_refunded', true);
            if (!$payplusRefunded) {
                payplus_update_post_meta_object($order, array('payplus_refunded' => $order->get_total()));
            }
            ...
            if (is_wp_error($response)) {
                ...
                if ($res->results->status == "success" && $res->data->transaction->status_code == "000") {
                    $WC_PayPlus_Gateway->payplus_add_log_all($handle, print_r($response, true), 'completed');
    ...
    public function load_admin_assets()
    {
        ...
        if ($_GET['page'] === 'wc-orders' && $_GET['action'] === 'edit') {
            $order_id = $_GET['id'];
            $order = wc_get_order($order_id);
        ...
        wp_enqueue_style('payplus', PAYPLUS_PLUGIN_URL . 'assets/css/admin.min.css', [], time());
        ...
        wp_localize_script('wc-payplus-gateway-admin', 'payplus_script_payment',
            array('ajax_url' => admin_url('admin-ajax.php'),
    ...
    public function process_make_payment($order_id)
    {
        ...
        if ($OrderType != "Approval" and $OrderType != "Check") {
            $WC_PayPlus_Gateway->payplus_add_log_all($handle, 'Transaction Not J5 Or Changed to J4 After Charge');
            $order->add_order_note(sprintf(__('The charge in PayPlus already made. Please check your PayPlus account<br />Amount: %s %s', 'payplus-payment-gateway'), $charged_amount, $order->get_currency()));
            return false;
        }
        if ($amount == $order->get_total() and $charged_amount == 0) {
    ...
    public function payplus_after_refund($order_id, $refund_id)
    {
        ...
        $payplus_related_transactions = get_post_meta($order_id, 'payplus_related_transactions', true);
        ...
        if ($payment_method != ""
            ...
            if (!$invocie_api->payplus_get_create_invocie_manual()
                && floatval($amount)) {
